...
def fintech_funding(request):
    ...
    #fintech_funding = Fintech_Funding.objects.filter(owner=request.user)
    fintech_funding = Fintech_Funding.objects.all()

    for funding in fintech_funding:
        funding.amount = "${:,.2f}".format(funding.amount)
        
    paginator = Paginator(fintech_funding, 5)
    ...
    context = {
        ...
        'fintech_funding': fintech_funding,
        'page_obj': page_obj,
    }
    return render(request=request, template_name='Fintech_Data_Dashboard/fintech_funding.html', context=context)


#@login_required(login_url='/authentication/login')
def fintech_funding_add(request):
    categories = Category.objects.all()
